...
function Model() {
  // Try to load model, fail gracefully if not found
  try {
    const { scene } = useGLTF("/models/Meshy_AI_Cliffside_Sky_Villa_0516212452_texture.glb");
...
export default function CraftSection() {
  ...
  return (
    <section ref={sectionRef} id="craft" className="h-screen w-full bg-[var(--surface)] relative overflow-hidden flex items-center z-10">
      ...
      <div className="w-1/2 h-full absolute right-0 top-0 hidden md:block">
        <Canvas camera={{ position: [5, 2, 5], fov: 45 }}>
          <ambientLight intensity={0.5} />
          <directionalLight position={[10, 10, 5]} intensity={1.5} color="#F5EFE4" />
          <fog attach="fog" args={["#FFF8EE", 10, 30]} />
      ...
      <div className="w-full md:w-1/2 pl-12 md:pl-24 h-full flex flex-col justify-center">
